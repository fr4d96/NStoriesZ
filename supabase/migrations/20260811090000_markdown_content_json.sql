-- content_json is now a single Markdown text block
-- ([{"type":"markdown","text":"..."}], lib/validation/story.ts) instead of
-- the old paragraph/heading/quote/list/table/image block union. Images stay
-- reference-only: a non-standard `![[<mediaId>]]` embed token in the
-- Markdown text, never a raw URL (see lib/story/markdown-media.ts). This
-- migration updates save_revision_draft's image-reference-integrity check
-- (added in 20260808130000_content_json_image_blocks.sql) to extract those
-- tokens from the Markdown text via regexp_matches instead of walking
-- jsonb "image" blocks -- the guarantee is unchanged: every embedded
-- mediaId must already be attached to this revision in
-- story_revision_media (Engineering Rule 2/3 -- this SECURITY DEFINER RPC,
-- not the client-side Zod schema, is the non-bypassable boundary).
...
begin
  ...
  begin
    ...
    from (
      ...
      from regexp_matches(v_content_text, '!\[\[([0-9a-fA-F-]{36})\]\]', 'g') as m
    ) embedded
    where not exists (
      select 1
...
comment on function public.save_revision_draft(
  ...
  'Returns the authoritative new story.version (Prompt 4 Sub-phase 4). Extended in 20260811090000 to extract ![[mediaId]] embed tokens from content_json[0].text (Markdown) via regexp_matches and reject any not attached to this revision in story_revision_media -- see this migration''s header comment.';
