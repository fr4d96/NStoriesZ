-- Real bug, found live: app/sitemap.ts's build-time call to
-- list_published_stories() (its first-ever real caller -- the /stories
-- page it backs was a placeholder until this same prompt) raised Postgres
-- 42702 ("column reference \"story_id\" is ambiguous"). The lateral
-- consent-lookup subquery's `where story_id = s.id` is bare, and
-- list_published_stories()'s own `returns table (story_id uuid, ...)`
-- makes `story_id` an implicit PL/pgSQL variable in the whole function
-- body -- the exact bug class already fixed twice before in this codebase
-- (20260803091000, 20260804092500), just never triggered here because this
-- function had no real caller before app/sitemap.ts. Fixed by qualifying
-- the reference with the subquery's own table alias, same pattern as both
-- prior fixes.
