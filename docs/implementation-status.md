...
changes-requested deliberately sends the contributor nothing yet — that is a scope decision, not
a gap in the mechanism, and adding it is one more `elsif` in the trigger.

- **Created by a trigger, not by the RPCs.** `story_revisions_notify_status_change` fires
  `AFTER UPDATE OF revision_status`. Checked against the LIVE catalogue first: exactly two
  functions write `'submitted'` (`submit_revision_with_consent`) and `'approved'`
  (`finalize_story_publication`), so editing both would have worked. The trigger works for the
  writer nobody has written yet, which is the point — a future offline-consent or bulk path
  cannot forget to notify. The submitting actor is excluded from their own fan-out.
- **Recipient re-derived from the story, never passed in.** `self_submitted` →
  `stories.owner_user_id`; `editorial_import` → `contributors.linked_user_id` — the same
  partition `list_my_stories()` uses. An imported story whose contributor never signed up gets no
  row rather than a null recipient. Not `coalesce(owner, linked)`: the source-kind partition
  exists so an import's `owner_user_id` is never an authority over it.
- **"Needs review" stops being true once reviewed.** When a revision leaves `submitted` by any
  decision, every still-unread `story_submitted` row for it is marked read. Three moderators, one
  reviews: the other two do not keep an unread badge pointing at a finished review — a badge you
  learn to ignore is worse than none.
- **Same access model as every story-domain table.** RLS on, zero policies, grants revoked;
  `list_my_notifications(p_limit)`, `count_my_unread_notifications()` and
  `mark_my_notifications_read(p_ids)` are SECURITY DEFINER and keyed on `auth.uid()`. Passing
  someone else's ids to mark-read changes 0 rows and reveals nothing. Verified live after `db
push`: the three RPCs are `authenticated`-only; the trigger function and the table are reachable
  by nobody.
- **One deliberate deviation from the deletion policy:** the story/revision FKs are
  `on delete cascade`. A notification is an inbox item derived from `moderation_actions`, not the
  audit record itself; `restrict` would have made `delete_draft_story()` a fourth enumeration site
  for a table with nothing to preserve. `scripts/rls-test-cleanup.sql` still lists it explicitly,
  because `replica` mode turns cascades off along with the constraints they belong to.
- **App side.** `components/notifications/notification-bell.tsx` beside the avatar in
  `SiteHeader` (both breakpoints), `ContributorNav` and `StaffNav`. Client-side reads for the same
  reason `SiteHeader` already reads identity client-side — public pages stay cacheable. Count on
  mount, on tab-visible, and every 60s while visible; the list only when the panel opens; mark-read
  is optimistic. `lib/notifications/notification-view.ts` maps kind → route
  (`/moderation/stories/<revision>` / `/stories/<slug>`), so the DB stores no URLs.

**Verified:** 19 new unit/RTL tests (helper 10, bell 9); 9 new RLS-suite cases drive the real
create → submit → approve flow and assert who was told, who was not (editor, owner-on-submit,
staff-on-publish), that the admin cannot mark the moderator's row read, and that the admin's
unopened row auto-reads on approval. `npm run verify` exits 0 — 1007/1007. In the Browser pane:
bell renders on a public page, a contributor page and at 375px with no console errors; the panel
at 375px first overflowed the LEFT edge by 9px (anchored to the bell, which sits left of the
avatar) — now `-right-12`, i.e. anchored to the avatar's edge, measured at 39–359px inside 375.
The signed-in pane account is an ordinary contributor with an empty inbox, so the populated state
was verified by tests, not by eye.

**Found on the way, fixed here:** `.claude/worktrees/epic-faraday-d62958/` — a leftover isolated
worktree with its own `node_modules` and `.next` — was breaking `npm run verify` in THREE tools
at once: vitest walked into it and every copied test failed on a second React (`reading
'useContext'`), Prettier flagged 133 of its files, and ESLint hit its `.next` chunks. All three
now exclude `.claude/worktrees/`. The worktree itself is untouched (clean, same commit as main);
`git worktree remove` it if nothing owns it.

**2026-09-11 — the unit suite's phantom PDF failures were a timeout budget, not a bug.**
